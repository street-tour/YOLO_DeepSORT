...
def risk_cluster_map():
    import pandas as pd
    import matplotlib.pyplot as plt
    from sklearn.cluster import KMeans

    # 경고 위치 로그 파일 경로
    file_path = "risk_zone_log.csv"
    ...
    # 중복 좌표 제거
    df_unique = df[['x', 'y']].drop_duplicates()

    # 클러스터링 수행
    n_clusters = 3
    kmeans = KMeans(n_clusters=n_clusters, random_state=0).fit(df_unique[['x', 'y']])
    ...
    plt.title("risk location clustering (KMeans)")
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig("risk_cluster_map.png")
    plt.show()
...
def shrink_bbox(bbox, width_ratio=0.7, height_ratio=1.0):
    x1, y1, x2, y2 = bbox
    w = x2 - x1
    h = y2 - y1
    cx = (x1 + x2) / 2
...
while cap.isOpened():
    ...
    for box in results.boxes.data.tolist():
        ...
        # 시각화 및 중심 계산용 bbox는 축소 적용
        vis_bbox = bbox
        if class_id == CLASS_PERSON:
            vis_bbox = shrink_bbox(bbox, width_ratio=0.8, height_ratio=1.0)
        elif class_id == CLASS_FORKLIFT:
            vis_bbox = shrink_bbox(bbox, width_ratio=0.6, height_ratio=1.0)
    ...
    for track in tracks:
        ...
        if det_class == CLASS_FORKLIFT:
            adjusted_bbox = shrink_bbox(bbox, width_ratio=0.7)
    ...
    for track in tracks:
        ...
        if class_id == CLASS_PERSON:
            x1, y1, x2, y2 = map(int, shrink_bbox(bbox, width_ratio=0.8))
        elif class_id == CLASS_FORKLIFT:
            x1, y1, x2, y2 = map(int, shrink_bbox(bbox, width_ratio=0.6))
        else:
