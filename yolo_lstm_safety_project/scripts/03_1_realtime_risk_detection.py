...
# 임계값 자동 반영 여부
APPLY_RECOMMENDED_THRESHOLDS = True
...
while cap.isOpened():
    ...
    moving_forklifts = [(tid, positions[tid]) for tid, buf in track_buffers.items()
                        if classes.get(tid) == CLASS_FORKLIFT and len(buf) == sequence_length and is_moving(buf)]
    ...
    for person_bbox in external_persons:
        ...
        for _, forklift_pos in moving_forklifts:
    ...
    for person_bbox in external_persons:
        ...
        for forklift_id, buffer in track_buffers.items():
            ...
            forklift_pred_real = scaler.inverse_transform(np.concatenate([forklift_pred_scaled, [0, 0]]).reshape(1, -1))[0][:2]
    ...
    cv2.putText(frame, f"RT:{DISTANCE_THRESHOLD_REALTIME:.1f}px PR:{DISTANCE_THRESHOLD_PREDICTED:.1f}px",
